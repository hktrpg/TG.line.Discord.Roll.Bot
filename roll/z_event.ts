...
var rollDiceCommand = async function(
    ...
    switch (true) {
        ...
        case /(^[.]event$)/i.test(mainMsg[0]) && /^add$/i.test(mainMsg[1]) && /^\S+$/.test(mainMsg[2]): {
            ...
            if (!events || !(events as any).MainData || !(events as any).eventName) {
...
var mainSplit = await analysicDetail((events as any).MainData);
            ...
            var listDatas = {
    title: (events as any).eventName,
    userID: userid,
    userName: displaynameDiscord || displayname || '',
    detail: mainSplit,
    expName: (events as any).expName || '',
    chainTitle: (events as any).eventChain || ''
};
    ...
    userID: userid,
    title: {
        $regex: new RegExp('^' + convertRegex((events as any).eventName) + ', "i")
    }
};
            try {
    ...
    eventList: {
        title: (events as any).eventName,
        eventID: (tempMain as any)._id
    }
};
            if (!(tempMain as any)._id) {
                rply.text = '新增事件失敗'
                return rply;
            }
            try {
                ...
                if (!temp) {
                    ...
                    var findEventId = temp.eventList.findIndex(((obj: any) => obj.eventID == (tempMain as any)._id));
                    if (findEventId >= 0) {
    ...
    eventID: (tempMain as any)._id
            ...
            //增加資料庫
//檢查有沒有重覆
rply.text = '新增/修改事件 - ' + (tempMain as any).title + '\n經驗值的名稱: ' + (tempMain as any).expName + '\n';
            rply.text += ((tempMain as any).chainTitle) ? `系列名稱: ${(tempMain as any).chainTitle}\n` : '';
            for (let index = 0; index < (tempMain as any).detail.length; index++) {
                rply.text += '類型:' + (tempMain as any).detail[index].result + ' 內容: ' + (tempMain as any).detail[index].event + '\n';
    ...
    $pull: {
        eventList: {
            ...
            //增加資料庫
//檢查有沒有重覆
rply.text = '刪除事件成功: ' + (doc as any).title;
            return rply;
        }
        ...
        case /(^[.]evt$)/i.test(mainMsg[0]) && /^\S+$/i.test(mainMsg[1]): {
            {
                ...
                let eventMode = '';
                let eventList = [];
                if ((targetEventName as any).match(/^random$/i)) {
                    eventMode = 'random';
                } else {
