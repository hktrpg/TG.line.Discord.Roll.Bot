...
// 只用環境變數 SOCKS5 控制是否啟用 socks-proxy-agent
if (process.env.SOCKS5) {
    const { SocksProxyAgent } = require('socks-proxy-agent');
    const agent = new SocksProxyAgent(process.env.SOCKS5);
    ...
    console.log('[Telegram] Using SOCKS5 proxy:', process.env.SOCKS5);
} else {
    TGclient = new Bot(process.env.TELEGRAM_CHANNEL_SECRET);
