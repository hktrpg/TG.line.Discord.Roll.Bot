        ...
        case /(^[.](r|)rap(\d+|)$)/i.test(mainMsg[0]) && /^add$/i.test(mainMsg[1]) && /^(?!(add|del|show)$)/ig.test(mainMsg[2]):
            {    //
                ...
                try {
                    ...
                    console.error('randomans #331 mongoDB error: ', (error as any).name, (error as any).reson);
                }
                return rply;
            }
        ...
        case /(^[.](r|)ras(\d+|)$)/i.test(mainMsg[0]) && /^add$/i.test(mainMsg[1]) && /^(?!(add|del|show)$)/ig.test(mainMsg[2]):
            {    //
                ...
                try {
                    ...
                    rply.text = '新增失敗'
                    console.error('randomans #463 mongoDB error: ', (error as any).name, (error as any).reson);
                }
                return rply;
            }
    ...
    async function replacer(first: any, second: any) {
        ...
        switch (true) {
            ...
            case /^allgp.name$/i.test(second):
                ...
                num = (num < 1) ? 0 : num;
                temp = (temp && temp[num] && (temp[num] as any).name) ? (temp[num] as any).name : ' ';
                return temp || ' ';
            // * {allgp.name} <---隨機全GP其中一人名字
