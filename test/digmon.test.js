...
describe('Digimon Evolution Path Finding Tests', () => {
  ...
  describe('幼年期1 -> 超究極體 (Stage 1 -> Stage 7)', () => {
    const testCases = [
      { from: "水母獸", to: "奧米加獸茲瓦特", expectedPath: ["水母獸", "柏古獸", "小惡魔獸", "惡魔獸", "吸血魔獸", "究極吸血魔獸", "究極吸血魔獸X"] },
    ...
    testCases.forEach((testCase, index) => {
      test(`Test ${index + 1}: ${testCase.from} -> ${testCase.to}`, () => {
        ...
        // Check if any path contains the expected evolution chain
        const hasValidPath = paths.some(path => {
          return path.length >= 2 && 
                 path[0].name === testCase.from && 
                 path[path.length - 1].name === testCase.to;
        });
  ...
  describe('成熟期 -> 成熟期 (Stage 4 -> Stage 4)', () => {
    ...
    testCases.forEach((testCase, index) => {
      test(`Test ${index + 1}: ${testCase.from} -> ${testCase.to}`, () => {
        ...
        expect(fromDigimon).toBeDefined();
        expect(toDigimon).toBeDefined();
        expect(fromDigimon.stage).toBe("4");
        expect(toDigimon.stage).toBe("4");
  ...
  describe('超究極體 -> 超究極體 (Stage 7 -> Stage 7)', () => {
    const testCases = [
      ...
      { from: "阿爾法獸王龍劍", to: "究極吸血魔獸X", expectedPath: ["阿爾法獸王龍劍", "阿爾法獸", "多路戰龍獸", "雷電獸", "加利獸", "電鼠獸", "水母獸", "柏古獸", "小惡魔獸", "惡魔獸", "吸血魔獸", "究極吸血魔獸", "究極吸血魔獸X"] },
