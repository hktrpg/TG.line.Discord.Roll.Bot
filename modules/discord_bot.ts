...
function channelFilter(channel: any) {
...
client.on('guildCreate', async guild => {
	...
	let channel = keys.find(channel => {
		return (channel as any).type === 'GUILD_TEXT' && (channel as any).permissionsFor(guild.me).has('SEND_MESSAGES');
	});
	...
	if (channel) {
		...
		await (channel as any).send({ embeds: [text] });
	}
})
...
// @ts-expect-error TS(2693): 'any' only refers to a type, but is being used as ... Remove this comment to see the full error message
	const list = await schema.roleReact.findOne({ messageID: reaction.message.id, groupid: any: reaction.message.guildId }).catch(error => console.error('discord_bot #817 mongoDB error: ', error.name, error.reson))
	if (!list || list.length === 0) return;
	const detail = list.detail;
...
// @ts-expect-error TS(2531): Object is possibly 'null'.
					...
					console.error('Discord GET ERROR:  SendtoID: ', (e as any).message, replyText);
				}
		}
	}
	else {
...
// @ts-expect-error TS(2531): Object is possibly 'null'.
				...
				if ((e as any).message !== 'Cannot send messages to this user') {
					console.error('Discord  GET ERROR:  SendToReply: ', (e as any).message, replyText);
				}
			}
	}
...
// @ts-expect-error TS(2531): Object is possibly 'null'.
				...
				if ((e as any).message !== 'Missing Permissions') {
					console.error('Discord  GET ERROR: SendToReplychannel: ', (e as any).message, replyText, channelid);
				}
			}
...
// @ts-expect-error TS(2304): Cannot find name 'exports'.
		...
		if (groupid && message.channel.permissionsFor(message.member) && message.channel.permissionsFor(message.member).has((Permissions as any).FLAGS.MANAGE_CHANNELS)) {
			userrole = 2
		}

		if (message.member && message.member.permissions.has((Permissions as any).FLAGS.ADMINISTRATOR)) {
			userrole = 3
		}
...
// @ts-expect-error TS(2304): Cannot find name 'exports'.
		...
		if ((rplyVal as any).requestRollingCharacter) await handlingRequestRollingCharacter(message, (rplyVal as any).requestRollingCharacter);
		if ((rplyVal as any).requestRolling) await handlingRequestRolling(message, (rplyVal as any).requestRolling, displaynameDiscord);
		if ((rplyVal as any).buttonCreate) (rplyVal as any).buttonCreate = await handlingButtonCreate(message, (rplyVal as any).buttonCreate);
		if ((rplyVal as any).roleReactFlag) await roleReact(channelid, rplyVal)
		if ((rplyVal as any).newRoleReactFlag) await newRoleReact(message, rplyVal)
		if ((rplyVal as any).discordEditMessage) await handlingEditMessage(message, rplyVal)

		if ((rplyVal as any).myName) await repeatMessage(message, rplyVal);
		if ((rplyVal as any).myNames) await repeatMessages(message, rplyVal);

		if ((rplyVal as any).sendNews) sendNewstoAll(rplyVal);

		if ((rplyVal as any).sendImage) sendBufferImage(message, rplyVal, userid)
...
// @ts-expect-error TS(2580): Cannot find name 'process'. Do you need to install... Remove this comment to see the full error message
			if (process.env.mongoURL && (rplyVal as any).text && isNew) {
		...
		if ((rplyVal as any).state) {
			(rplyVal as any).text += '\n' + (await count());
			(rplyVal as any).text += '\nPing: ' + Number(Date.now() - message.createdTimestamp) + 'ms';
			(rplyVal as any).text += await getAllshardIds();
		}

		if (groupid && rplyVal && (rplyVal as any).LevelUp) {
			await SendToReplychannel({ replyText: `<@${userid}>\n${(rplyVal as any).LevelUp}`, channelid });
		}

		if ((rplyVal as any).discordExport) {
			message.author.send({
    content: '這是頻道 ' + message.channel.name + ' 的聊天紀錄',
    files: [
        "./tmp/" + (rplyVal as any).discordExport + '.txt'
    ]
});
		}
		if ((rplyVal as any).discordExportHtml) {
			if (!link || !mongo) {
				message.author.send({
    content: '這是頻道 ' + message.channel.name + ' 的聊天紀錄\n 密碼: ' +
        (rplyVal as any).discordExportHtml[1],
    files: [
        "./tmp/" + (rplyVal as any).discordExportHtml[0] + '.html'
    ]
});
    ...
    link + ':' + port + "/app/discord/" + (rplyVal as any).discordExportHtml[0] + '.html');
			}
		}
    ...
    message,
    statue: (rplyVal as any).statue,
    quotes: (rplyVal as any).quotes,
    buttonCreate: (rplyVal as any).buttonCreate
};
