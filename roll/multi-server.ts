    ...
    switch (true) {
        ...
        case /^create$/i.test(mainMsg[1]) && /^\S/.test(mainMsg[2]): {
            try {
                ...
                const role = channel.permissionsFor(v).has((Permissions as any).FLAGS.MANAGE_CHANNELS);
                if (!role) return;
                ...
                await schema.multiServer.findOneAndUpdate({ guildID: channel.guildId }, { channelid: mainMsg[2], multiId, guildID: channel.guildId, guildName: channel.guild.name, channelName: channel.name }, { upsert: true }).catch((error: any) => {
        ...
        case /^join$/i.test(mainMsg[1]) && /^\S/.test(mainMsg[2]) && /^\S/.test(mainMsg[3]): {
            try {
                ...
                await schema.multiServer.findOneAndUpdate({ guildID: channel.guildId }, { channelid: mainMsg[3], multiId: mainMsg[2], guildID: channel.guildId, guildName: channel.guild.name, channelName: channel.name }, { upsert: true }).catch((error: any) => {
