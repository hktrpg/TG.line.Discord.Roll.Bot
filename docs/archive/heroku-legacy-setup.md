# Archived: Heroku / RoboYabaso legacy setup guide

> **Status:** Historical only. Do not follow for new deployments.
>
> Current self-hosting: copy `.env.copy` → `.env`, then `yarn install` / `yarn start`,
> or see [docs/DOCKER_SETUP.md](../DOCKER_SETUP.md).
>
> Moved from root `README.md` (section previously labeled「以下為舊版說明,將會廢棄或修改」).
