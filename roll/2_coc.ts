...
以後CC擲骰將 ${(result.switch) ? '會' : '不會'}進行紀錄`
		...
		console.error(`dpRecordSwitch ERROR ${(error as any).message}`);
		return '發生錯誤';
	}
}
 ...
 * @param {年齡} text01 
 */
// @ts-expect-error TS(7006): Parameter 'text01' implicitly has an 'any' type.
...
function sc(mainMsg) {
	...
	switch (true) {
		...
		case rollDice > san: {
			...
			if (lossSan) {
				lossSan = (lossSan as any).match(/\d+$/);
				let nowSan = ((san - lossSan) < 0) ? 0 : san - lossSan;
				return `San Check\n1d100 ≦ ${san}\n擲出:${rollDice} → 失敗!\n失去${lossSan}點San\n現在San值是${nowSan}點`
			} else
				return `San Check\n1d100 ≦ ${san}\n擲出:${rollDice} → 失敗!\n但不需要減少San`
