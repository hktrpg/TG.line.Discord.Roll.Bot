...
records.get('trpgDatabase', (msgs: any) => {
    (trpgDatabasefunction as any).trpgDatabasefunction = msgs;
});
records.get('trpgDatabaseAllgroup', (msgs: any) => {
    (trpgDatabasefunction as any).trpgDatabaseAllgroup = msgs;
});
    ...
    switch (true) {
        ...
        case /(^[.]db$)/i.test(mainMsg[0]) && /^add$/i.test(mainMsg[1]) && /^(?!(add|del|show)$)/ig.test(mainMsg[2]):
            ...
            if (groupid && userrole >= 1 && mainMsg[3]) {
                if ((trpgDatabasefunction as any).trpgDatabasefunction)
                    for (let i = 0; i < (trpgDatabasefunction as any).trpgDatabasefunction.length; i++) {
                        if ((trpgDatabasefunction as any).trpgDatabasefunction[i].groupid == groupid) {
                            if ((trpgDatabasefunction as any).trpgDatabasefunction[0] && (trpgDatabasefunction as any).trpgDatabasefunction[0].trpgDatabasefunction[0]) {
                                if ((trpgDatabasefunction as any).trpgDatabasefunction[i].trpgDatabasefunction.length >= limit) {
                                    rply.text = '關鍵字上限' + limit + '個\n支援及解鎖上限 https://www.patreon.com/HKTRPG\n';
                                    return rply;
                                }
                                for (let a = 0; a < (trpgDatabasefunction as any).trpgDatabasefunction[i].trpgDatabasefunction.length; a++) {
                                    if ((trpgDatabasefunction as any).trpgDatabasefunction[i].trpgDatabasefunction[a].topic == mainMsg[2]) {
                                        checkifsamename = 1
                                    }
                                }
                            }
        ...
        case /(^[.]db$)/i.test(mainMsg[0]) && /^del$/i.test(mainMsg[1]) && /^all$/i.test(mainMsg[2]):
            //刪除資料庫
            if (groupid && mainMsg[2] && (trpgDatabasefunction as any).trpgDatabasefunction && userrole >= 2) {
                for (let i = 0; i < (trpgDatabasefunction as any).trpgDatabasefunction.length; i++) {
                    if ((trpgDatabasefunction as any).trpgDatabasefunction[i].groupid == groupid) {
                        let temp = (trpgDatabasefunction as any).trpgDatabasefunction[i];
        ...
        case /(^[.]db$)/i.test(mainMsg[0]) && /^del$/i.test(mainMsg[1]) && /^\d+$/i.test(mainMsg[2]):
            //刪除資料庫
            if (groupid && mainMsg[2] && (trpgDatabasefunction as any).trpgDatabasefunction && userrole >= 1) {
                for (let i = 0; i < (trpgDatabasefunction as any).trpgDatabasefunction.length; i++) {
                    if ((trpgDatabasefunction as any).trpgDatabasefunction[i].groupid == groupid && mainMsg[2] < (trpgDatabasefunction as any).trpgDatabasefunction[i].trpgDatabasefunction.length && mainMsg[2] >= 0) {
                        let temp = (trpgDatabasefunction as any).trpgDatabasefunction[i];
                ...
                if ((trpgDatabasefunction as any).trpgDatabasefunction)
                    for (let i = 0; i < (trpgDatabasefunction as any).trpgDatabasefunction.length; i++) {
                        if ((trpgDatabasefunction as any).trpgDatabasefunction[i].groupid == groupid) {
                            ...
                            for (let a = 0; a < (trpgDatabasefunction as any).trpgDatabasefunction[i].trpgDatabasefunction.length; a++) {
                                temp = 1;
                                rply.text += ((a % 2 && a != 1) || a == 0) ? ("\n") + a + '. ' + (trpgDatabasefunction as any).trpgDatabasefunction[i].trpgDatabasefunction[a].topic : '       ' + a + '. ' + (trpgDatabasefunction as any).trpgDatabasefunction[i].trpgDatabasefunction[a].topic;
                            }
                        }
                    }
        ...
        case /(^[.]db$)/i.test(mainMsg[0]) && /\S/i.test(mainMsg[1]) && /^(?!(add|del|show)$)/ig.test(mainMsg[1]):
            ...
            if (groupid) {
                let temp = 0;
                if ((trpgDatabasefunction as any).trpgDatabasefunction && mainMsg[1])
                    for (let i = 0; i < (trpgDatabasefunction as any).trpgDatabasefunction.length; i++) {
                        if ((trpgDatabasefunction as any).trpgDatabasefunction[i].groupid == groupid) {
                            //rply.text += '資料庫列表:'
                            for (let a = 0; a < (trpgDatabasefunction as any).trpgDatabasefunction[i].trpgDatabasefunction.length; a++) {
                                if ((trpgDatabasefunction as any).trpgDatabasefunction[i].trpgDatabasefunction[a].topic.toLowerCase() == mainMsg[1].toLowerCase()) {
                                    temp = 1
                                    rply.text = (trpgDatabasefunction as any).trpgDatabasefunction[i].trpgDatabasefunction[a].topic + '\n' + (trpgDatabasefunction as any).trpgDatabasefunction[i].trpgDatabasefunction[a].contact;
        ...
        case /(^[.]dbp$)/i.test(mainMsg[0]) && /^add$/i.test(mainMsg[1]) && /^(?!(add|del|show)$)/ig.test(mainMsg[2]):
            //if (!mainMsg[2]) return;
            if (rply && (trpgDatabasefunction as any).trpgDatabaseAllgroup && mainMsg[2])
                if (rply && (trpgDatabasefunction as any).trpgDatabaseAllgroup && (trpgDatabasefunction as any).trpgDatabaseAllgroup[0] && (trpgDatabasefunction as any).trpgDatabaseAllgroup[0].trpgDatabaseAllgroup[0]) {
                    if ((trpgDatabasefunction as any).trpgDatabaseAllgroup[0].trpgDatabaseAllgroup.length > 100) {
                        rply.text = '只可以有100個關鍵字啊'
                        return rply;
                    }
                    for (let i = 0; i < (trpgDatabasefunction as any).trpgDatabaseAllgroup.length; i++) {
                        for (let a = 0; a < (trpgDatabasefunction as any).trpgDatabaseAllgroup[i].trpgDatabaseAllgroup.length; a++) {
                            if ((trpgDatabasefunction as any).trpgDatabaseAllgroup[i].trpgDatabaseAllgroup[a].topic.toLowerCase() == mainMsg[2].toLowerCase()) {
        ...
        case /(^[.]dbp$)/i.test(mainMsg[0]) && /^show$/i.test(mainMsg[1]):
            ...
            if ((trpgDatabasefunction as any).trpgDatabaseAllgroup)
                for (let i = 0; i < (trpgDatabasefunction as any).trpgDatabaseAllgroup.length; i++) {
                    rply.text += '資料庫列表:'
                    for (let a = 0; a < (trpgDatabasefunction as any).trpgDatabaseAllgroup[i].trpgDatabaseAllgroup.length; a++) {
                        tempshow = 1;
                        rply.text += ((a % 2 && a != 1) || a == 0) ? ("\n") + a + '. ' + (trpgDatabasefunction as any).trpgDatabaseAllgroup[i].trpgDatabaseAllgroup[a].topic : '      ' + a + '. ' + (trpgDatabasefunction as any).trpgDatabaseAllgroup[i].trpgDatabaseAllgroup[a].topic;
        ...
        case /(^[.]dbp$)/i.test(mainMsg[0]) && /\S/i.test(mainMsg[0]) && /^(?!(add|del|show)$)/ig.test(mainMsg[1]):
            ...
            if ((trpgDatabasefunction as any).trpgDatabaseAllgroup && mainMsg[1])
                for (let i = 0; i < (trpgDatabasefunction as any).trpgDatabaseAllgroup.length; i++) {
                    for (let a = 0; a < (trpgDatabasefunction as any).trpgDatabaseAllgroup[i].trpgDatabaseAllgroup.length; a++) {
                        if ((trpgDatabasefunction as any).trpgDatabaseAllgroup[i].trpgDatabaseAllgroup[a].topic.toLowerCase() == mainMsg[1].toLowerCase()) {
                            temp2 = 1
                            rply.text = (trpgDatabasefunction as any).trpgDatabaseAllgroup[i].trpgDatabaseAllgroup[a].topic + '\n' + (trpgDatabasefunction as any).trpgDatabaseAllgroup[i].trpgDatabaseAllgroup[a].contact;
    ...
    async function replacer(first: any, second: any) {
        ...
        switch (true) {
            ...
            case /^allgp.title$/i.test(second):
                ...
                if (!temp) return ' ';
                if ((temp as any).Title.length == 0) {
                    // @ts-expect-error TS(2304): Cannot find name 'exports'.
                    (temp as any).Title = exports.z_Level_system.Title();
                }
                temp2 = await (temp as any).Title.filter(function (item: any) {
    return item;
});
            ...
            case /^my.exp$/i.test(second):
                ...
                if (!temp || !temp2 || !(temp2 as any).EXP) return ' ';
                return (temp2 as any).EXP || ' ';
            ...
            case /^my.title$/i.test(second):
                ...
                temp = await findGp(groupid, userid, displayname, displaynameDiscord, membercount);
                temp2 = await findUser(groupid, userid);
                if (!temp || !temp2 || !(temp2 as any).Level || !(temp as any).Title) return ' ';
                //   let userTitle = await this.checkTitle(userlevel, trpgLevelSystemfunction.trpgLevelSystemfunction[i].Title);
// @ts-expect-error TS(2304): Cannot find name 'exports'.
return (await exports.z_Level_system.checkTitle((temp2 as any).Level, (temp as any).Title)) || ' ';
            case /^my.level$/i.test(second):
                //* {my.level}<---顯示擲骰者等級
                temp2 = await findUser(groupid, userid);
                if (!temp2 || !(temp2 as any).Level) return ' ';
                return (temp2 as any).Level || ' ';
