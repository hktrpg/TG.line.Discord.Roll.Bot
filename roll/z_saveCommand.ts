...
records.get('trpgCommand', (msgs: any) => {
    (trpgCommandfunction as any).trpgCommandfunction = msgs;
})
    ...
    switch (true) {
        ...
        case /(^[.]cmd$)/i.test(mainMsg[0]) && /^add$/i.test(mainMsg[1]) && /^(?!(add|del|show)$)/ig.test(mainMsg[2]):
            ...
            if (groupid && userrole >= 1 && mainMsg[3] && mainMsg[2] && mainMsg[3].toLowerCase() != ".cmd") {
                if ((trpgCommandfunction as any).trpgCommandfunction)
                    for (let i = 0; i < (trpgCommandfunction as any).trpgCommandfunction.length; i++) {
                        if ((trpgCommandfunction as any).trpgCommandfunction[i].groupid == groupid) {
                            if ((trpgCommandfunction as any).trpgCommandfunction[i].trpgCommandfunction.length >= 30) {
                                rply.text = '關鍵字上限' + limit + '個\n支援及解鎖上限 https://www.patreon.com/HKTRPG\n';
                                return rply;
                            }
                            if ((trpgCommandfunction as any).trpgCommandfunction[0] && (trpgCommandfunction as any).trpgCommandfunction[0].trpgCommandfunction[0])
                                for (let a = 0; a < (trpgCommandfunction as any).trpgCommandfunction[i].trpgCommandfunction.length; a++) {
                                    if ((trpgCommandfunction as any).trpgCommandfunction[i].trpgCommandfunction[a].topic == mainMsg[2]) {
                                        checkifsamename = 1
                                    }
                                }
                        }
        ...
        case /(^[.]cmd$)/i.test(mainMsg[0]) && /^del$/i.test(mainMsg[1]) && /^all$/i.test(mainMsg[2]):
            //刪除資料庫
            if (groupid && mainMsg[2] && (trpgCommandfunction as any).trpgCommandfunction && userrole >= 2) {
                for (let i = 0; i < (trpgCommandfunction as any).trpgCommandfunction.length; i++) {
                    if ((trpgCommandfunction as any).trpgCommandfunction[i].groupid == groupid) {
                        temp = (trpgCommandfunction as any).trpgCommandfunction[i];
                        (temp as any).trpgCommandfunction = [];
        ...
        case /(^[.]cmd$)/i.test(mainMsg[0]) && /^del$/i.test(mainMsg[1]) && /^\d+$/i.test(mainMsg[2]):
            //刪除資料庫
            if (groupid && mainMsg[2] && (trpgCommandfunction as any).trpgCommandfunction && userrole >= 1) {
                for (let i = 0; i < (trpgCommandfunction as any).trpgCommandfunction.length; i++) {
                    if ((trpgCommandfunction as any).trpgCommandfunction[i].groupid == groupid && mainMsg[2] < (trpgCommandfunction as any).trpgCommandfunction[i].trpgCommandfunction.length && mainMsg[2] >= 0) {
                        temp = (trpgCommandfunction as any).trpgCommandfunction[i];
                        (temp as any).trpgCommandfunction.splice(mainMsg[2], 1);
                        records.settrpgCommandfunction('trpgCommand', temp, () => {
                            records.get('trpgCommand', (msgs: any) => {
                                (trpgCommandfunction as any).trpgCommandfunction = msgs;
                            })
            ...
            if ((trpgCommandfunction as any).trpgCommandfunction)
                for (let i = 0; i < (trpgCommandfunction as any).trpgCommandfunction.length; i++) {
                    if ((trpgCommandfunction as any).trpgCommandfunction[i].groupid == groupid) {
                        ...
                        for (let a = 0; a < (trpgCommandfunction as any).trpgCommandfunction[i].trpgCommandfunction.length; a++) {
                            temp = 1
                            rply.text += ("\n") + a + '. ' + (trpgCommandfunction as any).trpgCommandfunction[i].trpgCommandfunction[a].topic + '\n' + (trpgCommandfunction as any).trpgCommandfunction[i].trpgCommandfunction[a].contact + '\n';
                        }
                    }
                }
        ...
        case /(^[.]cmd$)/i.test(mainMsg[0]) && /\S/i.test(mainMsg[1]) && /^(?!(add|del|show)$)/ig.test(mainMsg[1]):
            ...
            if ((trpgCommandfunction as any).trpgCommandfunction && mainMsg[1])
                for (let i = 0; i < (trpgCommandfunction as any).trpgCommandfunction.length; i++) {
                    if ((trpgCommandfunction as any).trpgCommandfunction[i].groupid == groupid) {
                        //rply.text += '資料庫列表:'
                        for (let a = 0; a < (trpgCommandfunction as any).trpgCommandfunction[i].trpgCommandfunction.length; a++) {
                            if ((trpgCommandfunction as any).trpgCommandfunction[i].trpgCommandfunction[a].topic.toLowerCase() == mainMsg[1].toLowerCase()) {
                                temp = 1
                                rply.text = (trpgCommandfunction as any).trpgCommandfunction[i].trpgCommandfunction[a].contact;
                                (rply as any).cmd = true;
                            }
                        }
                    }
                }
