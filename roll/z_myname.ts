...
// @ts-expect-error TS(2451): Cannot redeclare block-scoped variable 'limitAtArr... Remove this comment to see the full error message
    ...
    switch (true) {
        ...
        case /^\.myname+$/i.test(mainMsg[0]) && /^show$/i.test(mainMsg[1]): {
            ...
            if (groupid) {
                ...
                else (rply as any).myNames = result;
            }
        ...
        case /^\.myname$/i.test(mainMsg[0]): {
            ...
            rply.text = `已新增角色 - ${(myName as any).name}`;
            let myNames = await schema.myName.find({ userID: userid })

            if (groupid) { (rply as any).myNames = [showName(myNames, (myName as any).name)]; }
            else {
                rply.text += (showName(myNames, (myName as any).name) as any).content;
            }
            return rply;
        }
        case /^\.me\S+/i.test(mainMsg[0]): {
            ...
            (rply as any).myName = showMessage(myName, inputStr);
            return rply;
        }
        default: {
            break;
