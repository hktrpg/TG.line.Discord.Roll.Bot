...
// @ts-expect-error TS(2304): Cannot find name 'exports'.
	...
	if (!(rplyVal as any).text && !(rplyVal as any).LevelUp)
		return;
// @ts-expect-error TS(2580): Cannot find name 'process'. Do you need to install... Remove this comment to see the full error message
	if (process.env.mongoURL && (rplyVal as any).text && (await newMessage.newUserChecker(userid, "Line"))) {
		SendToId(userid, newMessage.firstTimeMessage());
	}


	if (roomorgroupid && rplyVal && (rplyVal as any).LevelUp) {
		if (displayname) {
			(rplyVal as any).text = (rplyVal as any).LevelUp + '\n' + (rplyVal as any).text;
...
// @ts-expect-error TS(7005): Variable 'TargetGMTempID' implicitly has an 'any[]... Remove this comment to see the full error message
			...
			break;
		default:
			if (displayname && rplyVal && (rplyVal as any).type != 'image') {
				//285083923223
				displayname = "@" + displayname;
				displayname += ((rplyVal as any).statue) ? ' ' + (rplyVal as any).statue + '\n' : "\n";
...
var replyMessagebyReplyToken = function (event, Reply) {
	...
	return client.replyMessage(event.replyToken, temp).catch(() => {
		if ((temp as any).type == 'image') {
			let tempB = {
    type: 'text',
    text: (temp as any).originalContentUrl
};
// @ts-expect-error TS(7006): Parameter 'err' implicitly has an 'any' type.
