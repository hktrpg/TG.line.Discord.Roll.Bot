...
const gameName = function () {
	return '【克蘇魯神話】 cc cc(n)1~2 ccb ccrt ccsu .dp .cc7build .cc6build .cc7bg .sc'
}
...
eg: .sc 50		.sc 50 1/1d3+1	.sc 50 1d10/1d100 .sc 1d10/1d100 

.sc (成功)/(失敗)
手動San Check模式，產生Discord Button

coc7版SanCheck記憶版
可以儲存San值並記錄你的SanCheck檢定結果，並自動進行加減

設定現時在群組中的SAN值 
.san set (SAN值) (名字:可選)

設定現時在頻道中的SAN值(頻道優先)
.san setChannel (SAN值) (名字:可選)

進行SanCheck檢定並記錄最近20次的SanCheck結果
.san (成功)/(失敗)					

.san show (名字:可選)
reply 或輸入名字，可以查看別人的SAN值及過去20次sc紀錄，不然就是自己的紀錄
