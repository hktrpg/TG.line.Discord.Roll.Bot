...
async function saveLog() {
    ...
    await schema.RealTimeRollingLog.findOneAndUpdate({}, {
        ...
        upsert: true
    }).catch((error: any) => console.error('log #90 mongoDB error: ', error.name, error.reson))
    //把擲骰的次數還原 為0
    resetLog();
...
async function getRecords() {
    let theNewData = await schema.RealTimeRollingLog.findOne({}).catch((error: any) => console.error('log # 131 mongoDB error: ', error.name, error.reson));
