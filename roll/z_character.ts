...
const rollDiceCommand = async function(
    ...
    switch (true) {
        ...
        case /(^[.]char$)/i.test(mainMsg[0]) && /^show$/i.test(mainMsg[1]):
            ...
            for (let index = 0; index < (doc as any).length; index++) {
                // @ts-expect-error TS(7053): Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        ...
        case /(^[.]char$)/i.test(mainMsg[0]) && /^add$/i.test(mainMsg[1]) && /^\S+$/.test(mainMsg[2]): {
            ...
            */
            ...
            filter = {
    id: userid,
    name: new RegExp('^' + convertRegex((Card as any).name) + ', "i")
};
            //取得本來的資料, 如有重覆, 以新的覆蓋
            doc = await schema.characterCard.findOne(filter);
            //把舊和新的合併
            if (doc) {
                (doc as any).name = (Card as any).name;
                // @ts-expect-error TS(2554): Expected 4 arguments, but got 3.
                (Card as any).state = await Merge((doc as any).state, (Card as any).state, 'name');
                // @ts-expect-error TS(2554): Expected 4 arguments, but got 3.
                (Card as any).roll = await Merge((doc as any).roll, (Card as any).roll, 'name');
                // @ts-expect-error TS(2554): Expected 4 arguments, but got 3.
                (Card as any).notes = await Merge((doc as any).notes, (Card as any).notes, 'name');
            }
            try {
...
filter = {
    ...
    cardId: (doc as any)._id
            ...
            //增加資料庫
//檢查有沒有重覆
rply.text = '刪除角色卡成功: ' + (doc as any).name;
            return rply;
        case /(^[.]char$)/i.test(mainMsg[0]) && /^button$/i.test(mainMsg[1]) && /^\S+$/.test(mainMsg[2]): {
            ...
            if (doc.roll)
                (rply as any).requestRollingCharacter = [handleRequestRolling(doc), doc.name, 'char'];
            return rply;
        }
        ...
        case /(^[.]ch$)/i.test(mainMsg[0]) && /^set$/i.test(mainMsg[1]) && /^\S+$/i.test(mainMsg[2]) && /^\S+$/i.test(mainMsg[3]):
            ...
            docSwitch = await schema.characterGpSwitch.findOne(
                filter);
            if (docSwitch && (docSwitch as any).cardId) {
                doc = await schema.characterCard.findOne({
    _id: (docSwitch as any).cardId
});
            } else {
                rply.text = "未有登記的角色卡, \n請輸入.char use 角色卡名字  \n進行登記"
            }
            if (doc) {
                ...
                (doc as any).state = await Merge((doc as any).state, useCard, 'name', true);
                (doc as any).roll = await Merge((doc as any).roll, useCard, 'name', true);
                (doc as any).notes = await Merge((doc as any).notes, useCard, 'name', true);
                try {
                    let a = await (doc as any).save();
                    if (a) {
                        let resutltState = (await findObject((doc as any).state, mainMsg[2])) || '';
                        let resutltNotes = (await findObject((doc as any).notes, mainMsg[2])) || '';
                        let resutltRoll = (await findObject((doc as any).roll, mainMsg[2])) || '';
                        if (resutltState) {
    ...
    _id: (docSwitch as any).cardId
        ...
        case /(^[.]ch$)/i.test(mainMsg[0]) && /^button$/i.test(mainMsg[1]): {
            ...
            if (docSwitch && docSwitch.cardId) {
                ...
                if (doc.roll)
                    (rply as any).requestRollingCharacter = [handleRequestRollingChMode(doc), doc.name, 'ch'];
            }
    ...
    _id: (docSwitch as any).cardId
            ...
            rply.characterName = (doc as any).name;
            return rply;
        default:
            break;
...
async function mainCharacter(doc: any, mainMsg: any) {
    ...
    if (Object.keys(findRoll).length > 0) { //https://stackoverflow.com/questions/33631041/javascript-async-await-in-replace
//ref source
...
tempRply.characterReRollItem = await replaceAsync((findRoll as any).itemA, /\{(.*?)\}/ig, await myAsyncFn);
        // @ts-expect-error TS(2551): Property 'characterReRollItem' does not exist on t... Remove this comment to see the full error message
        tempRply.characterReRollItem = await replaceAsync(tempRply.characterReRollItem, /\[\[(.*?)\]\]/ig, await myAsyncFn2);
        tempRply.characterReRollName = (findRoll as any).name;
