...
// @ts-expect-error TS(2451): Cannot redeclare block-scoped variable 'regex'.
    ...
    switch (true) {
        ...
        case /^\.roleReact$/i.test(mainMsg[0]) && /^add$/i.test(mainMsg[1]): {
            ...
            if (list) {
                ...
                (rply as any).newRoleReactFlag = true;
                (rply as any).newRoleReactMessageId = checkName.messageID;
                (rply as any).newRoleReactDetail = checkName.detail;
                return rply;
            }
