...
var rollDiceCommand = async function(
    ...
    switch (true) {
        ...
        case /^html$/i.test(mainMsg[1]):
            ...
            var tempB = key;
            await fs.writeFile(dir + channelid + '_' + hour + minutes + seconds + '_' + randomLink + '.html', newValue); (rply as any).discordExportHtml = [
    tempA + '_' + randomLink,
    tempB
];
        ...
        case /^txt$/i.test(mainMsg[1]): {
            ...
            await fs.writeFile(dir + channelid + '_' + hour + minutes + seconds + '.txt', data); (rply as any).discordExport = channelid + '_' + hour + minutes + seconds;
            rply.text += `已私訊你 頻道  ${discordMessage.channel.name}  的聊天紀錄
