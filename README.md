...
1. 複製 `.env.copy` 為 `.env`，填入各平台 Token 與 MongoDB 設定。
2. `yarn install` 後 `yarn start`（或使用 Docker，見 [docs/DOCKER_SETUP.md](docs/DOCKER_SETUP.md)）。
3. 更多文件見 [`docs/`](docs/)。

舊版 Heroku／mlab 教學已移至 [docs/archive/heroku-legacy-setup.md](docs/archive/heroku-legacy-setup.md)（僅供歷史參考，請勿用於新部署）。
