...
class CreateCult {
	...
	leaderPosition() {
		return CreateCult.LeaderPosition[rollbase.Dice(10) - 1];
	}
	characteristics() {
		return CreateCult.characteristics[rollbase.Dice(4) - 1];
	}
	skill() {
		let skill = [];
		let skillNum = rollbase.Dice(5) + 1;
		for (let index = 0; index < skillNum; index++) {
			skill.push(CreateCult.Skill[rollbase.Dice(6) - 1]);
		}
		return skill;
	}
	description() {
		return CreateCult.Description[rollbase.Dice(6) - 1];
	}
	personality() {
		return CreateCult.Personality[rollbase.Dice(6) - 1];
	}
	spells() {
		let spells = [];
		let spellsNum = rollbase.Dice(3) + 1;
		for (let index = 0; index < spellsNum; index++) {
			spells.push(CreateCult.Spells[rollbase.Dice(6) - 1]);
		}
		return spells;
	}
	sourcesOfPower() {
		return CreateCult.SourcesOfPower[rollbase.Dice(6) - 1];
	}
	cultGoals() {
		return CreateCult.CultGoals[rollbase.Dice(6) - 1];
	}
	cultGoalsMeans() {
		return CreateCult.CultGoalsMeans[rollbase.Dice(6) - 1];
	}
	...
	static characteristics = [[75, 70, 65, 60, 55, 50, 40, 35],
	...
	//手指
	static means = [
		`1 犧牲： 為了某種目的，教派信徒犧牲活的人類和/或動物。這可能是為了證明他們的忠誠，作為某種力量的來源，或是大計劃的一部分。`,
		`2 儀式： 為了進行一種或多種形式的儀式，需要一小群或大量的參與者。儀式可能完全為教派服務，或者是滿足Cthulhu的意志以完成某事（無論大小）。`,
		`3 集結： 可能是收集知識（神祕學典籍）, 特定的資源，甚至是特定的人。教派必須為了Cthulhu只知的理由收集“某物”。`,
		`4 服務： 通過服務神祕神靈—與Cthulhu有連結的一個， 教派正在滿足Cthulhu的意志。作為回報，教派將以某種方式得到實體的幫助。或者，教派已深陷於此實體之中，必須服從其命令。也許這個實體與Cthulhu沒有實際的連結，但它宣稱有連結只是為了對教派獲得更多的權力。`,
		`5 創造
